...
import com.android.rtmp.rtmp.utils.Utf8Validator;
/**
...
public class WebSocketReader extends Thread {
    ...
    public WebSocketReader(Handler master, SocketChannel socket, WebSocketOptions options, String threadName) {
        ...
        mFrameBuffer = ByteBuffer.allocateDirect(options.getMaxFramePayloadSize());
            ...
            if (mFrameBuffer.position() >= 2) {
                ...
                if (mFrameBuffer.position() >= header_len) {
                    ...
                    if (payload_len1 == 126) {
                        ...
                        if ((0x80 & mFrameBuffer.get(i)) != 0) {
                            throw new WebSocketException("invalid data frame length (> 2^63)");
                        }
                        payload_len = ((long) (0xff & mFrameBuffer.get(i)) << 56) |
     ...
     *
     * @param messageS1 message from server to client for handshake
     */
    protected void onHandshake(byte[] messageS1) {
        notify(new WebSocketMessage.RawTextMessage(messageS1));
        notify(new WebSocketMessage.ServerHandshake(true));
    }
...
//        for (int pos = mFrameBuffer.position(); pos >= 0; --pos) {
//            if (mFrameBuffer.get(pos) == 0x0d &&
...
//            }
//        }
        byte[] all = new byte[Constants.HANDSHAKE_SIZE_SERVER];
        mFrameBuffer.get(all, 0, Constants.HANDSHAKE_SIZE_SERVER);
        byte S0 = all[0];
        byte[] S1 = new byte[Constants.HANDSHAKE_SIZE];
        System.arraycopy(all, 1, S1, 0, Constants.HANDSHAKE_SIZE);
        mFrameBuffer.clear();
        onHandshake(S1);
        return true;
    }
    ...
    public void run() {
        ...
        try {
            ...
            do {
                ...
                mSocket.read(mFrameBuffer);
                int len = mFrameBuffer.position();
                if (len > 0) {
